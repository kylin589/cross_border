$(function () {
    $("#jqGrid").jqGrid({
        url: baseURL + 'order/domesticlogistics/list',
        datatype: "json",
        colModel: [			
...
var vm = new Vue({
	...
	methods: {
		...
		saveOrUpdate: function (event) {
			var url = vm.domesticLogistics.domesticLogisticsId == null ? "order/domesticlogistics/save" : "order/domesticlogistics/update";
			$.ajax({
				type: "POST",
			    url: baseURL + url,
		...
		del: function (event) {
			...
			confirm('确定要删除选中的记录？', function(){
				$.ajax({
					type: "POST",
				    url: baseURL + "order/domesticlogistics/delete",
                    contentType: "application/json",
		...
		getInfo: function(domesticLogisticsId){
			$.get(baseURL + "order/domesticlogistics/info/"+domesticLogisticsId, function(r){
